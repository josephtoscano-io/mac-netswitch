#!/bin/bash
# mac-netswitch — auto Wi-Fi toggle
# originally from https://gist.github.com/albertbori/1798d88a93175b9da00b

# rate limiting, run at most every second
if [ -f "/var/tmp/prev_mac_netswitch_run" ]; then
    prev_mac_netswitch_run=`cat /var/tmp/prev_mac_netswitch_run`
    prev_mac_netswitch_run_po=$(($prev_mac_netswitch_run + 1))
    current=`date +%s`

    if (( $prev_mac_netswitch_run_po > $current )); then
        exit 0
    fi
fi

date +%s > /var/tmp/prev_mac_netswitch_run
...
# Grab the names of the adapters.
air_name=`networksetup -listnetworkserviceorder | sed -En 's/^\(Hardware Port: (Wi-Fi|AirPort).* Device: (en[0-9]+)\)$/\2/p'`
# Get all en* interfaces except Wi-Fi — covers any wired adapter regardless of name (e.g. Plugable docks)
eth_names=`networksetup -listnetworkserviceorder | sed -En 's/^\(Hardware Port: .* Device: (en[0-9]+)\)$/\1/p' | grep -v "^${air_name}$"`
